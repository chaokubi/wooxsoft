package com.mip.software.common.interceptor;
...
public class WebPaginationInterceptor implements Interceptor {

	private final static Log logger = LogFactory.getLog(WebPaginationInterceptor.class);
    private static final ObjectFactory DEFAULT_OBJECT_FACTORY = new DefaultObjectFactory();
    private static final ObjectWrapperFactory DEFAULT_OBJECT_WRAPPER_FACTORY = new DefaultObjectWrapperFactory();
    private static String defaultDialect = "mysql"; // 数据库类型(默认为mysql)
    private static String defaultPageSqlId = ".*PageData$"; // 需要拦截的ID(正则匹配)
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		pageSqlId = configuration.getVariables().getProperty("webPageSqlId");
		...
		if (mappedStatement.getId().matches(pageSqlId)) {
			...
			if (parameterObject == null) {
				...
				// 重设分页参数里的总页数等
				setPagination(sql, connection, mappedStatement, boundSql,
						page);
			}
		}
		// 将执行权交给下一个拦截器
	 ...
	 * @param page
	 */
	private void setPagination(String sql, Connection connection,
...
//				* page.getPageSize());
		...
		String endrow = String.valueOf(page.getPageSize());
